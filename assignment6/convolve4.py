...
def do_conv(img, size):
    ...
    def convolve_naive4(input_params, kernel_params, num_channels, hyperparams):
        in_h = input_params[0]
        in_w = input_params[1]
            ...
            for hh in range(np.shape(output)[0]):
                ...
                for ww in range(np.shape(output)[1]): 
                    ...
                    for k_hh in range(k_h):
                        input_ch_ww = input_ch[hh_s + k_hh, :]
                        for k_ww in range(k_w): 
                            running_total += input_ch_ww[ww_s + k_ww] * kernel_ch[k_hh, k_ww]
    ...
    output, time_taken = convolve_naive4(input_params, kernel_params, num_channels,hyperparams)
